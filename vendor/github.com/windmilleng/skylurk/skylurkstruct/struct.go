...
// Package skylurkstruct defines the Skylark 'struct' type,
// an optional language extension.
package skylurkstruct
...
// 		"struct":  skylark.NewBuiltin("struct", skylurkstruct.Make),
// 	}
//
func Make(_ *skylurk.Thread, _ *skylurk.Builtin, args skylurk.Tuple, kwargs []skylurk.Tuple) (skylurk.Value, error) {
	if len(args) > 0 {
...
// key/value pairs in kwargs.  (Each kwargs[i][0] must be a skylurk.String.)
func FromKeywords(constructor skylurk.Value, kwargs []skylurk.Tuple) *Struct {
	if constructor == nil {
	...
	for _, kwarg := range kwargs {
		k := string(kwarg[0].(skylurk.String))
		v := kwarg[1]
...
func FromStringDict(constructor skylurk.Value, d skylurk.StringDict) *Struct {
	if constructor == nil {
		panic("nil constructor")
	}
	s := &Struct{
...
var (
	_ skylurk.HasAttrs  = (*Struct)(nil)
	_ skylurk.HasBinary = (*Struct)(nil)
)

// ToStringDict adds a name/value entry to d for each field of the struct.
func (s *Struct) ToStringDict(d skylurk.StringDict) {
...
func (s *Struct) Constructor() skylurk.Value { return s.constructor }

func (s *Struct) Type() string        { return "struct" }
func (s *Struct) Truth() skylurk.Bool { return true } // even when empty
func (s *Struct) Hash() (uint32, error) {
	// Same algorithm as Tuple.hash, but with different primes.
	var x, m uint32 = 8731, 9839
	for _, e := range s.entries {
		namehash, _ := skylurk.String(e.name).Hash()
		x = x ^ 3*namehash
		y, err := e.value.Hash()
		if err != nil {
			return 0, err
...
func (x *Struct) Binary(op syntax.Token, y skylurk.Value, side skylurk.Side) (skylurk.Value, error) {
	if y, ok := y.(*Struct); ok && op == syntax.PLUS {
		if side == skylurk.Right {
			x, y = y, x
		}

		if eq, err := skylurk.Equal(x.constructor, y.constructor); err != nil {
		...
		z := make(skylurk.StringDict, x.Len()+y.Len())
...
func (s *Struct) Attr(name string) (skylurk.Value, error) {
	...
	case "to_json", "to_proto":
		return skylurk.NewBuiltin(name, func(thread *skylurk.Thread, fn *skylurk.Builtin, args skylurk.Tuple, kwargs []skylurk.Tuple) (skylurk.Value, error) {
			var buf bytes.Buffer
			if name == "to_json" {
				writeJSON(&buf, s)
			} else {
			...
			return skylurk.String(buf.String()), nil
		}), nil
	}
...
func writeJSON(out *bytes.Buffer, v skylurk.Value) error {
	...
	case skylurk.Indexable: // Tuple, List
		out.WriteByte('[')
		for i, n := 0, skylurk.Len(v); i < n; i++ {
			if i > 0 {
				out.WriteString(", ")
			}
	...
	case *Struct:
		...
		for i, e := range v.entries {
			...
			if err := writeJSON(out, skylurk.String(e.name)); err != nil {
				return err
			}
			out.WriteString(": ")
...
func (x *Struct) CompareSameType(op syntax.Token, y_ skylurk.Value, depth int) (bool, error) {
	y := y_.(*Struct)
	switch op {
	case syntax.EQL:
...
func structsEqual(x, y *Struct, depth int) (bool, error) {
	...
	for i, n := 0, x.Len(); i < n; i++ {
		if x.entries[i].name != y.entries[i].name {
			return false, nil
		} else if eq, err := skylurk.EqualDepth(x.entries[i].value, y.entries[i].value, depth-1); err != nil {
			return false, err
		} else if !eq {
			return false, nil
		}
